...
CenteredGridView {
    ...
    NavigableDialog {
        ...
        // The PIN is generated by the CLIENT (us) and shown to the user.
        // The user enters it on the host's web UI to complete pairing.
        // This matches how Vibepollo/Apollo/Sunshine all work:
        //   client shows PIN → user enters it on server → challenge handshake.
        // The OTP extension adds otpauth=SHA256(pin+salt) for extra server-side
        // verification, but the PIN direction is always client→server.
        ...
        onOpened: {
            // Generate a fresh random 4-digit PIN each time the dialog opens.
            var n = Math.floor(Math.random() * 10000)
            generatedPin = ("000" + n).slice(-4)

            // Kick off pairing immediately so the host receives the request
            // and shows the "Incoming Pairing Request" notification right away.
            // The handshake waits on the server side until the user enters
            // the PIN in the host's web UI.
        ...
        ColumnLayout {
            ...
            // PIN display — large and prominent so it's easy to read and type
            ColumnLayout {
                Layout.fillWidth: true
                spacing: 4
            ...
            // Step-by-step instructions for Vibepollo / Apollo / Sunshine
            ColumnLayout {
                Layout.fillWidth: true
                spacing: 4
                ...
                Label {
                    text: qsTr("2.  In the host web UI — find\n    \"Pair Client\" and enter:")
                    Layout.fillWidth: true
                    wrapMode: Text.Wrap
                }

                Label {
                    // Repeat PIN inline so the user doesn't have to scroll up
                ...
                Label {
                    text: qsTr("3.  This dialog closes automatically when pairing completes.")
                    Layout.fillWidth: true
                    wrapMode: Text.Wrap
                }
            }
            ...
            Label {
                text: qsTr("The pairing request has been sent — the host notification\nshould appear within a few seconds.")
                Layout.fillWidth: true
                wrapMode: Text.Wrap
                color: "#aaaaaa"
                font.pointSize: 9
