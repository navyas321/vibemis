...
**AppImage:** Download the latest release (including prereleases) from:
https://github.com/navyas321/vibemis/releases

The AppImage must be from commit `308c2c12` or newer (branch
`fix/quickmenu-clipboard-otp`, built after 2026-05-28 10:00 UTC).
Fix branches publish as prereleases — use the `/releases` API (not
`/releases/latest` which skips prereleases) to get the newest build.
...
# Download latest AppImage — uses /releases (not /releases/latest) so
# fix-branch prereleases are included.
