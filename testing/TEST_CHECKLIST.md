...
- [x] **test58** — Show host software version in PC details (P3.13) — **PR #78** — base `vibemis-main` — ☑ **PASS*** (report PR #123; source+serverInfo: appversion=7.1.431.-1 → "Host Software Version: 7.1.431.-1" in DetailsRole; Apollo Version suppressed when empty; no crash; *UI dialog not directly opened — screen lock on test device; recommend visual follow-up) → merged
