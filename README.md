...
- **One-toggle VRR frame pacing** — flip "Enable VRR (beta)" in Settings and everything else is automatic: V-Sync enables itself, the FPS target derives from your panel's refresh rate, and the Vulkan presenter is selected for you — no scripts, no env vars, no config editing; sessions that can't sustain VRR fall back to fixed pacing with a visible notice, and the whole feature is validated with real streaming sessions on device before every release
