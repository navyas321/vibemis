...
- **One-toggle VRR frame pacing** — flip "Enable VRR" in Settings and everything else is automatic: V-Sync enables itself, the FPS target derives from your panel's refresh rate, and the Vulkan presenter is selected for you — no scripts, no env vars, no config editing. The timing core is aligned with upstream Nonary VRR10 (GPU queue synchronization, AI-tuned timing parameters, display-scaled latch policy); sessions that can't sustain VRR fall back to fixed pacing with a visible notice
- **Settings that make sense** — every option lives where you'd look for it: display, decoding, and HDR together under Video, connection options under Streaming, app behavior under App & UI — reviewed item-by-item with an automated zero-drop check so reorganizations never lose a setting
- **Clean, crackle-free audio on PipeWire handhelds** — Vibemis ships a real SDL2 audio runtime (PulseAudio path) instead of the SDL3+sdl2-compat native-PipeWire stack other current builds use, and that difference is CI-guarded so upstream merges can't silently lose it
- **Updates that can't brick your install** — the self-updater stages the new AppImage beside the old one and swaps with an atomic rename (never rewriting the running file), keeps a rollback copy, and survives crashes mid-download
- **Kept current** — upstream moonlight-qt changes are pulled in through staged, device-verified quarterly rebases (see the release notes for what each cycle adopted), with a CI pipeline on every push
